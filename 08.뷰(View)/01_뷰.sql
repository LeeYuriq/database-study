/*
	View : 실제 데이터를 저장하지 않고, 기본 테이블의 쿼리 결과를 기반으로
		생성되는 가상의 테이블
*/
USE sqldb;
...
DESC view_user; 	-- 테이블처럼 구조 확인 가능 (가상테이블)
DESC usertbl; 		-- 구조 확인(실제테이블)
-- 실제(물리적) 테이블에서는 기본키 제약조건이 존재하지만
-- 가상(논리적) 테이블에서는 기본키 제약조건이 존재하지 않음. 
-- 컬럼명, 타입 동일

-- 가상 테이블과 실제 테이블이 조건이 동일할 경우에만 삽입 가능
INSERT INTO view_user VALUES ('ABC', '에비씨'); -- 삽입 안됐음 

-- 제약조건이 일치할 경우 DML 수행됨 -> 실제 테이블에 반영
...
/*
	CREATE / DROP 으로 DB에서 모든 생성/삭제 함
*/

/*
	뷰 사용의 장점
    1. 데이터 보안 강화 : 사용자가 실제 테이블에 접근하지 못하게 하여
		민감한 정보 노출을 방지
	2. 데이터 접근 단순화 : 여러 테이블을 조인하거나 복잡한 셀렉트문의 결과를
		반복적으로 사용해야 할 경우
