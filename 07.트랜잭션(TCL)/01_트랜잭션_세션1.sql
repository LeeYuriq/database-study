/*
    ...
    - 트랜젝션 내의 연산은 모두 성공하거나 모두 실패 해야함. (All or Nothing)
	...
	- 트랜잭션 제어 언어 (TCL)
     ...
     - ROLLBACK : 트랜잭션 내 모든 연산을 취소. 이전상태로 복구.**트랜잭션을 종료**
     - SAVEPOINT : 트랜잭션 내 특정 중간지점을 설정.
     - ROLLBACK TO SAVEPOINT : 설정한 지점으로 롤백(복구), **트랜잭션을 종료하지 않음**
*/
...
SELECT * FROM accounts;   -- 커밋이 이루어지지 않은 부분 완료 상태 
...
/*
	트랜잭션이 시작되고, 커밋이 완료가 되지 않은 상태에서 세션이 종료되면 
    트랜잭션 시작 전으로 롤백
*/
...
-- 첫번째 거래지점까지 롤백하기 (트랜잭션을 종료하지 않는다.)
ROLLBACK TO P1;  
