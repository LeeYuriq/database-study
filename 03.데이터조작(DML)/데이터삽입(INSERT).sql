CREATE DATABASE insert_db;  -- DATABASE와 SCHEMA 같음.
USE insert_db;
...
DESCRIBE employees;		-- 스키마(데이터 구조) 확인 = DESC 와 같음
...
-- 컬럼명의 순서와 삽입할 데이터의 순서를 일치시켜야함.
-- 컬럼(속성)의 데이터 타입과 입력하는 데이터 탕비이 일치해야함
-- 위배 시 도메인 제약조건 위반으로 삽입 불가

INSERT INTO employees (emp_no, birth_date, first_name, last_name, gender, hire_date)
	VALUES ('문자열', '날짜아님', '길동', '홍', 'xxx', '2024-05-22');
-- (오류) 테이블 구조의 데이터타입과 맞지 않는 경우
...
-- DEFAULT 제약조건이 있는 경우 컬럼을 선택하지 않으면 기본갑싱 삽입
