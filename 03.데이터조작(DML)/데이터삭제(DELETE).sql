...
-- 조건에 맞는 모든 행을 삭제 => 147개의 행 삭제
DELETE FROM employees 
	WHERE first_name = 'Georgi';
    
-- 하나의 특정한 행만 삭제하고 싶을 경우 UNIQUE 속성, PRIMARY KEY
-- 고유값을 WHERE 조건 컬럼명으로 지정하여 삭제 => 1개의 행 삭제
DELETE FROM employees
	WHERE emp_no = '10002';
...
-- 다중 조건 삭제
DELETE FROM employees
	WHERE fitst_name = 'Jane' AND last_name = 'Pettey' LIMIT 1;
...
DROP TABLE big1;	-- DDL, 빠른속도, 테이블 없어짐
DELETE FROM big2;	-- DML, 한줄씩 데이터 삭제, 트랜잭션 발생, 테이블 존재(구조가 남아있음). -- 사용안하는게 좋음.
TRUNCATE TABLE big3; -- DDL, 빠른속도, 테이블 존재(구조가 남아있음)
