...
SELECT * 
FROM emp -- 테이블 1 (왼쪽 테이블 left table) 
INNER JOIN dept   -- 테이블 2 (오른쪽 테이블 right table) 
...
-- NATURAL JOIN 
-- 공통된 이름, 데이터타입을 가진 컬럼을 기준으로 자동으로등가 조인
...
-- LEFT JOIN에 추가되는 Rows (컬럼기준) 

SELECT DISTINCT deptono FROM dept;		
-- 오른쪽에는 존재 왼쪽에는 없는 것(40) 
-- RIGHT JOIN에 추가되는 Rows (컬럼기준)
...
-- 임시로 추가한 예시 데이터 삭제
DELETE FROM emp WHERE empno=9999;  -- 예시로 추가했던 ABC씨 삭제
...
-- CROSS JOIN 
-- 양쪽 테이블의 모든 행을 조인(두 테이블의 행을 곱한 개수를 반환)
-- 카티션 프로덕트
SELECT * FROM emp
CROSS JOIN dept;
