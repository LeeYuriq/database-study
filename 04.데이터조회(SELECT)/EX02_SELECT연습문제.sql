...
DESC city; 
SELECT Name, Population FROM city ORDER BY Population DESC LIMIT 10 OFFSET 10;

-- 문제 3: ORDER BY와 LIMIT, OFFSET 사용
-- 3. country 테이블에서 대륙(Continent)이 'Asia'인 나라들을 면적(SurfaceArea) 기준으로 내림차순 정렬하여 상위 5개의 나라를 조회하세요.
DESC country; 
SELECT Name, SurfaceArea FROM country WHERE Continent = 'Asia' ORDER BY SurfaceArea DESC LIMIT 5 OFFSET 0 ; 
