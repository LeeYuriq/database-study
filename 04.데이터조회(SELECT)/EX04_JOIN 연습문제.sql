...
SELECT f.title, f.title AS 영화제목, count(fa.actor_id) AS 영화별배우수
FROM film f
...
SELECT r.rental_id, s.store_id, sf.first_name AS '직원이름' , sf.last_name AS '직원 성'
FROM rental r
...
SELECT f.film_id, f.title, count(r.rental_id) AS '영화별대여횟수'
FROM film f
JOIN inventory i ON f.film_id = i.film_id
JOIN rental r ON i.inventory_id = r.inventory_id
GROUP BY f.film_id
ORDER BY 영화별대여횟수 DESC LIMIT 5;
...
SELECT c.customer_id, c.first_name, c.last_name, sum(p.amount)  AS '총 지불금액'
FROM customer c 
...
SELECT c.category_id, c.name, count(rental_id) AS '대여된 영화의 총 수'
 FROM category c
