...
/*
  - 클러스터 인덱스 : MySQL 에서는 기본키를 기준으로 클러스터 인덱스 자동 생성 
    - 테이블 당 단 하나만 존재 
...
-- 조회 후 Excution Plan을 확인하면 쿼리비용 1.00 
-- 쿼리 행 수는 Single Row (1 rows) (Full Table Scan 이 아님)
EXPLAIN SELECT * FROM emp WHERE emp_no = 100000; 
...
-- 단 한건의 데이터를 찾을 때도 여전희 Full Table Scan
SELECT * FROM emp 
...
SELECT * FROM emp WHERE first_name = 'Hiroyasu'; 	-- 239건
EXPLAIN SELECT * FROM emp WHERE first_name = 'Hiroyasu'; 
...
/*
	- 인덱스를 잘못 사용할 경우 오히려 성능이 저하 될 수 있으므로 주의 
      1. 데이터 삽입, 삭제, 수정 시 작업마다 실제 DB + 모든 인덱스를 업데이트해야함.
      2. 인덱스는 추가적인 디스크 공간을 사용 (불필요한 인덱스는 지양)
	- 인덱스를 생성해야 하는 경우
	  1. 인덱스는 열(컬럼)단위로 생성
      2. WHERE 절에서 사용 되는 컬럼에 인덱스를 생성 
      3. WHERE 절에서 '자주' 사용되는 컬럼에 인덱스 생성
